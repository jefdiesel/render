...
module.exports = (baseUrl, url, scanId, summary) => {
  // Fallback URLs if not provided in summary
  const pdfReportUrl = summary.reportUrls?.pdf || 
`${baseUrl.trim()}/reports/pdf/${scanId}.pdf`;
  const csvReportUrl = summary.reportUrls?.csv || 
`${baseUrl.trim()}/reports/csv/${scanId}.csv`;
  
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 
auto;">
      <div style="text-align: center; margin-bottom: 20px;">
        <img src="${baseUrl}/images/a11yscan-logo.svg" alt="A11yscan Logo" 
width="180" height="50" style="display: inline-block;">
      </div>
...
${summary.infoIssues}</li>
      ...
      <div style="text-align: center; margin: 30px 0;">
        <a href="${pdfReportUrl}" style="background-color: #4f46e5; color: 
white; padding: 12px 24px; text-decoration: none; border-radius: 5px; 
font-weight: bold; display: inline-block; margin-right: 10px;">Download PDF 
Report</a>
        <a href="${csvReportUrl}" style="background-color: #3b82f6; color: 
white; padding: 12px 24px; text-decoration: none; border-radius: 5px; 
font-weight: bold; display: inline-block;">Download CSV Report</a>
      </div>
      
      <p>This report highlights accessibility issues on your website that may 
prevent people with disabilities from using it effectively. Addressing these 
issues will help you:</p>
      ...
      <p>Your free report will be available for 7 days. For more comprehensive 
testing and ongoing monitoring, check out our <a href="${baseUrl}/#pricing" 
style="color: #4f46e5;">paid plans</a>.</p>
...
#e5e7eb; font-size: 12px; color: #6b7280; text-align: center;">
